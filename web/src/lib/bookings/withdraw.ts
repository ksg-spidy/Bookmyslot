...
export async function withdrawBooking(opts: {
  ...
  allowLockedSession?: boolean;
}): Promise<WithdrawBookingResult> {
  const { admin, stripe, playSessionId, userId, whatsappIdentityId, allowLockedSession } = opts;
  ...
  if (session.status !== "open" && !(allowLockedSession && session.status === "locked")) {
