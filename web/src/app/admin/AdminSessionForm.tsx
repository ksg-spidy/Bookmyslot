...
export function AdminSessionForm() {
  ...
  return (
    <form action={formAction} className="mt-4 grid max-w-xl gap-3">
      ...
      <label className="text-xs uppercase text-[#8b949e]">Booking fee (cents)</label>
      <input name="booking_fee_cents" type="number" defaultValue={1500} className={inp} required />

      <label className="text-xs uppercase text-[#8b949e]">Withdrawal fee (cents)</label>
      <input name="withdrawal_fee_cents" type="number" defaultValue={200} className={inp} required />
